...
async function windowsSearch(name, runtime) {
	if (await runtime.commandExists("es.exe")) try {
		return lines(decodeWindowsOutput(await runtime.execBuffer("es.exe", [
			"-n",
			String(100),
			"-w",
			name
	...
	try {
		return lines(decodeWindowsOutput(await runtime.execBuffer("powershell.exe", [
			"-NoProfile",
			"-NonInteractive",
			"-Command",
			script
...
async function shallowCandidates(item, roots) {
	...
	for (const root of roots) {
		...
		for (const entry of entries) {
			...
			let grandchildren;
			try {
				grandchildren = await readdir(directory, { withFileTypes: true });
			} catch {
				continue;
			}
